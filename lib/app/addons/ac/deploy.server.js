            ...
            for( viewId in binderMap ) {
                if( binderMap.hasOwnProperty( viewId ) ) {
                    if( binderMap[viewId].name ) {
                        ...
                        // FIX MOJ-326, delete configs in the binder map.---
                        clientConfig.binderMap[viewId] = Y.clone( binderMap[viewId] );
                        if( clientConfig.binderMap[viewId].children ) {
                            for( child in clientConfig.binderMap[viewId].children ) {
                                delete clientConfig.binderMap[viewId].children[child].config;
                            }
                        }
                        delete binderMap[viewId].config;
                        // end fix
                    }
                }
            }
