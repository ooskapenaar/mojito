...
OutputHandler.prototype = {
    ...
    flush: function(data, meta) {
        ...
        if( this.req.__timing && this.req.__timing.length > 3  ) {
            this.req.__timing.push( {mojitoOutput: finished} );
            this.logger.log( this.req.url + ' TIMING  ' +
                 this.req.__timing.reduce( function( logstr, item, idx ) {
                         return logstr + '| ' + Object.keys( item )[0] + ' ' + (finished - item[Object.keys( item )[0]]) + ' ms ago';
                     },
                     '' ), 'mojito', NAME );
        }
